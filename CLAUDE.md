...
2. **Standards pass** — restructure to the repo doc shape (direct start, dense sections, `## Quick recall` ending, pure GFM/no HTML, ```java blocks, no emojis). **Never drop a discussion point (especially pain points or struggles)** — cleanup means reorganize/rephrase/dedupe, not cut. *(Note: This rule explicitly overrides the "ruthless cutting" mandate in DocCreationStandard.md for temp.md imports).* Non-interview tangents go to a `## Good to know` section near the end instead of being removed. Avoid shortening the user's content; only true redundancy may be collapsed.
