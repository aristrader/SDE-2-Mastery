...
## Readable Leaf-Page Filenames

The current curriculum validator accepts only `index.md` pages. This keeps generated navigation and
module routing predictable, but makes individual source filenames opaque.

Later evaluate:

- Keep `index.md` for module landing pages.
- Permit named leaf pages such as `problem-statement.md` and `candidate-discussion.md`.
- Update route discovery, navigation generation, validation, and broken-link checks together.
- Migrate only where a descriptive filename improves source navigation; do not rename every page by
  default.

Do not implement this schema change until the content layout is otherwise stable.
