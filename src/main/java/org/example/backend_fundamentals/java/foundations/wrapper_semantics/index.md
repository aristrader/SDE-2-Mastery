...
Every primitive in Java (`int`, `boolean`, `double`, etc.) has a corresponding object wrapper class (`Integer`, `Boolean`, `Double`, etc.). The basic reason wrappers exist is covered with primitives; this page focuses on the object-semantics traps.
