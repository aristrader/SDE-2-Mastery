package org.example.backend_fundamentals.design_patterns.creational.builder.simple_builder;

import lombok.ToString;

/**
 * Immutable record of a candidate's compensation terms, demonstrating the
 * Effective Java-style Builder pattern (EJ Item 2).
 *
 * <p>Required fields ({@code salary}, {@code city}) are constructor arguments
 * of {@link Builder} and held {@code final}; optional bonuses default to
 * {@code 0} and are configured via fluent setters. The product's constructor
 * is private so the only path to a {@code JobOffer} is via
 * {@link Builder#build()}, where all validation runs.
 *
 * <p>See {@code BuilderBasic.md} for the full walkthrough.
 *
 * @see Builder
 */
@ToString
public final class JobOffer {
  ...
  /**
   * Fluent builder for {@link JobOffer}. Required fields are passed to the
   * constructor and held {@code final}; optional bonuses are configured via
   * fluent setters that return {@code this}. All validation runs in
   * {@link #build()}.
   */
  public static class Builder {
     ...
     *
     * @throws IllegalStateException if any single-field or cross-field rule
     *     is violated (salary {@code > 0}, city non-blank, bonuses
     *     {@code >= 0}, total bonuses {@code <=} salary)
     */
