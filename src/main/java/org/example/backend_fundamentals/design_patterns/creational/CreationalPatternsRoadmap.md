...
Then the pattern-specific docs in order:
1. `factory/simple_factory/SimpleFactory.md` — the non-GoF starting point
...
Then the variant-specific docs in order:
1. `builder/simple_builder/BuilderBasic.md` — hand-written EJ-style (required fields in constructor, validation in `build()`)
