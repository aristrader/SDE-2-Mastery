...
When Spring resolves an `@Autowired` injection point it applies these rules in order:

1. **By type** — find all beans assignable to the declared type. If exactly one, done.
