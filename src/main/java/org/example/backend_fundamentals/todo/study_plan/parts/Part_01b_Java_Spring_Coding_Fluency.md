...
| 2 | Stream collectors — groupingBy / toMap / joining / partitioningBy / counting downstream | 🔴 💼 🎯 | 45 min | [x] | [ ] | [ ] | [ ] | Structured practice created | 📖 `java/coding_fluency/streams/exercise/index.md` · 💻 Group orders by status map; build `id→order` map (hit the duplicate-key trap); join names with delimiter |
