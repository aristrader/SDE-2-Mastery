...
*(Exception: If you are processing a `temp.md` ChatGPT dump, follow the root `AGENTS.md` Temp Doc Import Workflow instead: do NOT cut these if they were actively discussed. Move them to a `## Good to know` section to preserve the user's learning context.)*
