...
| 9 | Cost vs performance tradeoffs — gp3 vs io2, Multi-AZ vs single-AZ for non-prod, Graviton vs Intel | 🟠 💼 🎯 | M | 1 hr | [ ] | [ ] | [ ] | [ ] |  |  |
...
3. **Q:** Database scaling — when to over-provision vs auto-scale?
   - **Why asked:** Capacity choice. Over-provisioning provides predictable low latency but wastes money during off-peak. Auto-scaling saves money but may suffer latency spikes during sudden burst traffic (due to connection pooling or storage scaling limits). Right choice: provision for baseline, scale or burst for peaks.
...
3. **Q:** Index write-amplification — what's the hidden cost?
   - **Gotcha:** Every secondary index you add requires an extra write operation for every insert/update. Add 5 indexes → 6× write cost and IOPS load. Common backend mistake: over-indexing for read performance without calculating the write capacity penalty and DB locking overhead.
...
- **Per-verification cost model for KYC platform** (~2.5 hrs) — break down a single KYC verification end-to-end. Compute cost per. Useful for product pricing + cost-optimization decisions.
- **Silent killers audit** (~1.5 hrs) — NAT GW, cross-AZ, inter-region, LB cross-zone. Walk through your current architecture identifying which apply, estimate savings from VPC endpoints / per-AZ NAT.
