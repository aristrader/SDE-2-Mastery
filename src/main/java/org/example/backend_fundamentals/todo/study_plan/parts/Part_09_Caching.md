...
|---|-------|------|------|------|------|---|----------|-------------|-------|-----------|
| 1 | Patterns — cache-aside, read-through, write-through, write-behind, refresh-ahead | 🔴 💼 🎯 | D | 2.5 hrs | [x] | [ ] | [ ] | [ ] | ~45 min (ChatGPT). Refresh-ahead pending; write-around covered as bonus | 📖 `system_design/caching/CachingAndDistributedCache.md` |
| 2 | Invalidation strategies — TTL, explicit, event-driven | 🔴 💼 🎯 | MP | 1 hr 45 min | [ ] | [x] | [ ] | [ ] | Partial: explicit delete-vs-update on write covered; TTL + event-driven strategies pending | 📖 `system_design/caching/CachingAndDistributedCache.md` (explicit only) · 💻 Warm-up: cache-aside with TTL using Spring `@Cacheable(unless=...)` + explicit eviction via `@CacheEvict` (15 min) |
...
|-------|-----------------------|----------------------|-------------|
| 🔴 MUST only (Sprint priority — 3-month plan) | ~10.25 hrs | ~0.93 wk | ~45 min so far (row 1 done; rows 2, 3 partial) |
| 🔴 + 🟠 HIGH (must + high — senior coverage) | ~21 hrs | ~1.9 wk | ~45 min so far (rows 6, 11 also partial) |
