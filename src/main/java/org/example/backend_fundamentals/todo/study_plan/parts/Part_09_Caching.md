...
|---|-------|------|------|------|------|---|----------|-------------|-------|-----------|
| 1 | Patterns — cache-aside, read-through, write-through, write-behind, refresh-ahead | 🔴 💼 🎯 | D | 1.5 hrs | [x] | [ ] | [ ] | [ ] | ~45 min (ChatGPT). Refresh-ahead pending; write-around covered as bonus | 📖 `system_design/caching/CachingAndDistributedCache.md` |
| 2 | Invalidation strategies — TTL, explicit, event-driven | 🔴 💼 🎯 | MP | 1.5 hrs | [ ] | [x] | [ ] | [ ] | Partial: explicit delete-vs-update on write covered; TTL + event-driven strategies pending | 📖 `system_design/caching/CachingAndDistributedCache.md` (explicit only) · 💻 Warm-up: cache-aside with TTL using Spring `@Cacheable(unless=...)` + explicit eviction via `@CacheEvict` (15 min) |
